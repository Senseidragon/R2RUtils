...
public class R2RHandler {
    ...
    public void checkUpdate(PlayerEvent.PlayerLoggedInEvent event) {
        if (R2RHandler.outdated) {
            event.player.addChatComponentMessage(new TextComponentString("An updated version of R2RUtils is available!"));
            event.player.addChatComponentMessage(new TextComponentString(R2RHandler.updates));
        }
    }
                ...
                if (Reference.gameMode == 0) {
                    ...
                    if (player.inventory.armorInventory[3] == null) {
                        ...
                        for (int i = posY + 2; i < 256; i++) {
                            ...
                            if (block != Blocks.AIR && block != Blocks.LEAVES && block != Blocks.LEAVES2) {
                        ...
                        if (blockCount > 6) {
                            player.addChatComponentMessage(new TextComponentString("The blocks over your head don't look too safe, wear a helmet! "));
                    ...
                    if (depth > offset) {
                        // Check for helm
                        if (player.inventory.armorInventory[3] == null) {
                            player.addChatComponentMessage(new TextComponentString("Without your helmet, you start to panic!"));
                            afflictPlayer(player);
                        }
                        ...
                        if (depth > offset + 10) {
                            ...
                            if (player.inventory.armorInventory[2] == null) {
                                player.addChatComponentMessage(new TextComponentString("You feel unprotected without your chest armor.  Your breathing becomes labored..."));
                                afflictPlayer(player);
                            }
                            ...
                            if (depth > offset + 20) {
                                ...
                                if (player.inventory.armorInventory[1] == null) {
                                    player.addChatComponentMessage(new TextComponentString("Your unborn children cry for protection as you find yourself pantless."));
                                    afflictPlayer(player);
                                }
                                ...
                                if (depth > offset + 30) {
                                    ...
                                    if (player.inventory.armorInventory[0] == null) {
                                        player.addChatComponentMessage(new TextComponentString("This far underground, a foot injury could prove fatal.  Put on some boots!"));
                                        afflictPlayer(player);
                                    }
                                }
                            }
    ...
    private void afflictPlayer(EntityPlayerMP player) {
        ...
        if (Reference.isBlind) {
            effect = new PotionEffect(MobEffects.BLINDNESS, 100, 0);
            player.addPotionEffect(effect);
        }
        // Nausea I potion effect
        if (Reference.isConfused) {
            effect = new PotionEffect(MobEffects.NAUSEA, 100, 0);
            player.addPotionEffect(effect);
        }
        // Slowness I potion effect
        if (Reference.isSlowed) {
            effect = new PotionEffect(MobEffects.SLOWNESS, 100, 0);
            player.addPotionEffect(effect);
        }
        // Mining Fatigue V potion effect
        if (Reference.isFatigued) {
            effect = new PotionEffect(MobEffects.MINING_FATIGUE, 100, 4);
            player.addPotionEffect(effect);
        }
    }
    ...
    private int blocksBetween(World world, int posX, int posY, int posZ, int surface) {
        ...
        for (int i = surface; i > posY; i--) {
            ...
            if (block.isBlockSolid(world, pos, EnumFacing.UP) &&
                    !block.isAir(block.getDefaultState(), world, pos) &&
                    !block.isLeaves(block.getDefaultState(),world, pos) &&
                    !block.isWood(world, pos)) between++;
        }
    ...
    private int getSurfaceYLevel(World world, int posX, int posY, int posZ) {
        ...
        for (int i = 255; i > posY; i--) {
            ...
            if (block.isBlockSolid(world, pos, EnumFacing.UP)) {
                BlockPos pos2 = new BlockPos(posX, i-1, posZ);
                Block block2 = world.getBlockState(pos2).getBlock();

                if (block2.isBlockSolid(world, pos2, EnumFacing.UP)) {
                    surfaceY = i;
                    break;
                }
            }
